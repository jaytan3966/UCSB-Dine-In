...
export const Navbar = ({topRef, reviewRef, menuRef, aboutRef} : NavbarProps) => {
    return (
        <div className="bg-[#1d2f54] p-4 flex justify-between items-center max-w-screen">
            ...
            <button className="font-bold text-1xl text-white px-4 hover:-translate-y-1 delay-150 duration-300" onClick={() => handleScrollDown(reviewRef)}>Reviews</button>
            <button className="font-bold text-1xl text-white px-4 hover:-translate-y-1 delay-150 duration-300" onClick={() => handleScrollDown(menuRef)}>Today's Menu</button>
