...
export const Navbar = ({ topRef, diningRef, aboutRef }: NavbarProps) => {
  return (
    <div className="bg-[#1d2f54] p-4 flex justify-between items-center max-w-screen">
      <button
        className="font-bold text-3xl text-[#ffce34]"
        onClick={() => scrollHome()}
      >
        UCSB Dine-In
      </button>
      <div className="flex justify-end">
        <button
          className="font-bold text-1xl text-white px-4 hover:-translate-y-1 delay-150 duration-300"
        ...
        <button
          className="font-bold text-1xl text-white px-4 hover:-translate-y-1 delay-150 duration-300"
          onClick={() => handleScrollDown(diningRef)}
        >
          Dining Commons
        </button>
        <button
